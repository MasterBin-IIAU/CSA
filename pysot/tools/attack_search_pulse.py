...
prob = 0.2
model_name = 'search_impulse_%f'%prob
SAVE_TXT = False
SAVE_IMG = True
save_img_root = '/home/masterbin-iiau/Desktop/Adv_search_region_complete/%s'%model_name
        ...
        for v_idx, video in enumerate(dataset):
            if args.video != '':
                ...
                if video.name != args.video:
                    continue
            if SAVE_IMG:
                save_img_path = os.path.join(save_img_root,video.name)
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == frame_counter:
                    ...
                    '''impulse'''
                    if SAVE_IMG:
                        outputs = tracker.track_impulse(img, prob,save_path=save_img_path,frame_id=idx)
                    else:
                        outputs = tracker.track_impulse(img,prob)
