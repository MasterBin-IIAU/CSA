...
from ..utils import overlap_ratio, success_overlap, success_error, success_overlap_bin
...
class OPEBenchmark:
    ...
    def eval_success(self, eval_trackers=None):
        ...
        return success_ret
    def eval_success_bin(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if hasattr(video, 'absent'):
                    gt_traj = gt_traj[video.absent == 1]
                    tracker_traj = tracker_traj[video.absent == 1]
                success_ret_[video.name] = success_overlap_bin(gt_traj, tracker_traj, n_frame)

                np.savetxt('/home/masterbin-iiau/Desktop/AdvTrack-project/iou/%s.txt'%video.name,
                           success_ret_[video.name], fmt='%f', delimiter=',')
